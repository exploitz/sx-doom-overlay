...
// sx-doom-overlay: doomgeneric is older than the chocolate-doom commit we
// vendored i_oplmusic.c from. Inline the enum that newer chocolate-doom
// adds in i_sound.h, so we don't have to fork doomgeneric for it.
typedef enum {
    opl_doom1_1_666,
    opl_doom2_1_666,
    opl_doom_1_9
...
static int M_remove(const char* path) { return remove(path); }
...
static void *I_OPL_RegisterSong(void *data, int len)
{
    ...
    // sx-doom-overlay: in-memory MIDI loading. Original chocolate-doom code
    // wrote a temp file to /tmp/doom.mid and read it back via MIDI_LoadFile —
    // both the Switch tmp dir absence AND the per-song-change SD I/O burst
    // were heap-fragmentation sources that crashed libtesla's HID poll
    // thread on level transitions. Now: if MUS, convert in memory via
    // ConvertMusBuffer; then MIDI_LoadBuffer parses directly from RAM.
...
retry:
    ...
    {
        // MUS → MIDI conversion in memory. Three-step heap dance to
        // minimize peak pressure during parse:
        //   1. mus2mid into a 64 KB-initial growable MEMFILE
        //   2. memcpy the used portion to a TIGHT malloc, free the streams
        //   3. parse from the tight buffer
        // After step 2, the 64 KB output buffer is back on the free list,
        // giving MIDI_LoadBuffer's per-track event arrays room to grow.
        // Was the cause of "MIDI_LoadBuffer NULL" on a fresh-heap warp to
        // E1M2 — the stream sat there occupying contiguous heap that the
        // event arrays needed.
        ...
        {
            doom_trace("RegisterSong: mem_fopen_* alloc failed");
        }
        else
        {
            const int mus_rc = mus2mid(instream, outstream);
            if (mus_rc != 0)
            {
                char buf[64];
                snprintf(buf, sizeof(buf),
                         "RegisterSong: mus2mid failed rc=%d (len=%d)",
                         mus_rc, len);
                doom_trace(buf);
            }
            else
            {
                ...
                else
                {
                    ...
                    if (!result)
                    {
                        char buf[80];
                        snprintf(buf, sizeof(buf),
                                 "RegisterSong: MIDI_LoadBuffer NULL (mus_in=%d "
                                 "midi_out=%zu)",
                                 len, (size_t)outbuf_len);
                        doom_trace(buf);
                    }
                }
            }
        }
        if (instream)  mem_fclose(instream);
...
// sx-doom-overlay: drop `const` to match doomgeneric's i_sound.h forward decl.
