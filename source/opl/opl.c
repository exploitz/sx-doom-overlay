...
// sx-doom-overlay: stripped of config.h, SDL.h, and platform-specific
// drivers (linux/win32/obsd ioperm-based hardware OPL access). Only the
// software-emulation path is kept, with the libnx driver registered.
...
extern opl_driver_t opl_libnx_driver;
...
// 48 kHz to match Switch audout's device rate; opl_libnx_driver renders at
// the same rate so we can submit OPL output directly without resampling.
...
// sx-doom-overlay: original OPL_Delay used SDL_mutex+SDL_cond to wait for a
// queued callback to fire after `us` microseconds. We don't have SDL; the
// only callers are OPL detection probes and one-time register-write timing
// during init. A plain real-time svcSleepThread gives at least the same
// elapsed wait, and our submit thread (running concurrently at 48 kHz) will
// have advanced the OPL clock past `us` before the sleep returns.
...
void OPL_Delay(uint64_t us)
{
    ...
    svcSleepThread((u64)us * 1000ULL);  // us → ns
}
